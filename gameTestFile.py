...
#call board class functions in this file
import pygame
import sys
...
def main():
    ...
    # Instantiate Board class into board variable that contain board member
    board = Board ;
    game_over = False
    x_turn = True

    msg="Tic-Tac-Toe"
    drawBoard(board.board, msg)
    ...
    while True:
        ...
        if event.type == QUIT:
            sys.exit()
        if coords and check_move_legal(coords, board.board) and not game_over:
            if two_player:
                next_step = board_to_step(*coords)
                if x_turn:
                    update_board(board.board, next_step, PLAYER_X)
                    x_turn = False
                else:
                    update_board(board.board, next_step, PLAYER_O)
                    x_turn = True
                drawBoard(board.board, msg)
                pygame.display.update()
            ...
            if not two_player:
                ...
                drawBoard(board.board, msg)
                pygame.display.update()
                minmax(board.board, 0)
                update_board(board.board, choice, PLAYER_O)

            result = check_win_game(board.board)
